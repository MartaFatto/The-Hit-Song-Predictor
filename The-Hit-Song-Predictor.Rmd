...
```{r}
# Further explore the distribution of instrumentalness according to the target
...
Compare the full logistic regression model with a regularization method in order to see which are the most relevant predictors. 
We choose **Lasso penalized logistic regression**
...
Before fitting the model, tune the hyperparameters `cost` and `gamma`. 
...
Fit the model on the train data to predict the test error rate
...
Tune the hyperparameters *shrinkage*, *tree depth* and *number of trees*. 
We tried with (`learning_rate = c(0.3, 0.1, 0.05, 0.01, 0.005)` and `interaction.depth = c(3, 5, 7, 9)`), but here we consider only some of those for efficiency. 
Perform 5-fold cross validation to choose the best ones
...
Fit the model with these hyperparameters and predict the test error rate
...
Tune the hyperparameters `mtry` and `ntree` which refers to the number of trees
...
No need to compute the test error with CV, look at the OOB classification error to estimate it
